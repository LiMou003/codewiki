...
export default function Dashboard() {
  ...
  const loadConfigFromCache = useCallback((repoUrl: string) => {
    ...
    // Only language is listed (setState setters are stable) so cached config reloads when language changes without exhaustive-deps noise
  }, [language]);
  ...
  const hasProjectsToDisplay = !projectsLoading && projects.length > 0;
          ...
          <section className="lg:col-span-2 space-y-4">
            <div className="card-modern p-6 shadow-custom">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                <div>
                  <p className="text-xs uppercase tracking-wide text-[var(--muted)]">{hasProjectsToDisplay ? t('projects.browseExisting') : t('home.welcomeTagline')}</p>
                  <h2 className="text-xl md:text-2xl font-bold text-[var(--foreground)]">
                    {hasProjectsToDisplay ? t('projects.existingProjects') : t('home.welcome')}
                  </h2>
                </div>
